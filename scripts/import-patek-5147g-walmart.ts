/**
 * Import Pre-Owned Patek Philippe 5147G Annual Calendar Moonphase (Walmart listing).
 *
 * Usage:
 *   npx tsx scripts/import-patek-5147g-walmart.ts
 *   npx tsx scripts/import-patek-5147g-walmart.ts --apply
 */
...
const PRODUCT_URL =
  "https://www.walmart.com/ip/Pre-Owned-Patek-Philippe-Annual-Calendar-Moonphase-5147G-18k-White-Gold-Diamond-Bezel-Ref-5147G-001/17195062225";

const PRICE_GHS = 2000;
const STOCK_PER_BRANCH = 1;

const BRAND = "Patek Philippe";
const MODEL = "Annual Calendar Moonphase 5147G Diamond Bezel";
const REFERENCE = "5147G-001";
const COLLECTION = "Complications";

const DESCRIPTION = `Pre-owned Patek Philippe Annual Calendar Moonphase reference 5147G-001 in 18 ct white gold with a diamond-set bezel.

This elegant complication displays day, date, month, and moon phase in a balanced dial layout — requiring only one manual correction per year (1 March). The manufacture self-winding calibre is finished to haute horlogerie standards and visible through the exhibition caseback.

**Specifications**
- Reference: 5147G-001
- Collection: Complications
- Case: 39 mm, 18 ct white gold
- Bezel: Diamond-set
- Dial: Annual calendar with moon phase aperture
- Bracelet: Alligator leather with white gold fold-over clasp
- Movement: Self-winding with annual calendar and moon phase
- Water resistance: 30 metres
- Condition: Pre-owned, professionally inspected`;

const CONDITION_REPORT = `Pre-owned haute horlogerie. White gold case with diamond bezel. Annual calendar moon phase. Leather strap with white gold deployant. Inspected and ready for sale.`;
...
async function main() {
  ...
  const imageSlug = slugify(`${brandSlug}-5147g-moonphase-diamond`);
  ...
  try {
    ...
    const data = {
      ...
      bottleDetail: "39 mm white gold · diamond bezel · moon phase · 30 m WR",
      bottleSize: 50,
      capType: "MAGNETIC" as const,
      liquidColor: null,
      longevity: "Power reserve per manufacture spec",
      bottleShape: "Round annual calendar",
