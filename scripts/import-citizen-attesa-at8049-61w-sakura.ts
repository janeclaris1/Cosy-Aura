/**
 * Import Citizen Attesa LIGHT in BLACK 2022 GREEN Limited Edition AT8049-61W
 *
 * Usage:
 *   npx tsx scripts/import-citizen-attesa-at8049-61w-sakura.ts
 *   npx tsx scripts/import-citizen-attesa-at8049-61w-sakura.ts --apply
 */
...
const PRODUCT_URL =
  "https://www.sakurawatches.com/citizen-attesa-light-in-black-2022-green-limited-edition-at8049-61w";

const PRICE_GHS = 699.99;
const STOCK_PER_BRANCH = 1;

const BRAND = "Citizen";
const MODEL = "Attesa Light in Black Green Limited Edition";
const REFERENCE = "AT8049-61W";
...
async function fetchProductImageUrls(productUrl: string, reference: string): Promise<string[]> {
  ...
  const refKey = reference.toUpperCase();
  const seen = new Set<string>();
  const urls: string[] = [];
  const re =
    /\/image\/cache\/catalog\/watches\/(?:[^"'\s>]+\/)*([^"'\s>]+\.(?:jpg|jpeg|png|webp))/gi;
  let match: RegExpExecArray | null;
  while ((match = re.exec(html)) !== null) {
    const filename = match[1];
    if (!filename.toUpperCase().includes(refKey)) continue;
    if (!filename.includes("1000x1000")) continue;
    if (seen.has(filename)) continue;
    seen.add(filename);
    urls.push(`https://www.sakurawatches.com/image/cache/catalog/watches/citizen/UPLOAD/${filename}`);
  }
...
const LEAD = `The Attesa Light in Black AT8049-61W is a 2022 limited edition built on Super Titanium with Duratect DLC — black, lightweight, and hard-wearing. Eco-Drive H800 pairs solar power with radio-controlled atomic sync for set-and-forget accuracy.

At 41.5 mm and 9.7 mm thick, the round case frames a black and green analog dial under sapphire crystal with clarity coating. Chronograph, perpetual calendar, world time, day/date, power reserve indicator, luminous hands and markers, and antimagnetic protection complete a travel-ready spec on a matching black Super Titanium bracelet with 100 metres of water resistance.`;
...
function buildDescription(): string {
  return rebuildWatchDescriptionFromRecord({
    ...
    bottleDetail:
      "41.5 mm Super Titanium · Duratect DLC · 9.7 mm thick · sapphire clarity · 100 m WR",
    liquidColor: "Black and green analog dial · luminous hands and markers",
    longevity: "Eco-Drive H800 solar · radio controlled · antimagnetic",
    bottleSize: 42,
    condition: "UNWORN",
    lead: LEAD,
    legacyDescription: `Specifications
- Reference: ${REFERENCE}
- Collection: Attesa Light in Black (2022 limited edition)
- Case: 41.5 mm Super Titanium with Duratect DLC, 9.7 mm thick
- Dial: Black and green analog
- Crystal: Sapphire with clarity coating
- Bracelet: Super Titanium with Duratect DLC
- Movement: Eco-Drive H800 solar quartz, radio controlled
...
const CONDITION_REPORT =
  "New limited edition. 41.5 mm Super Titanium case and bracelet with Duratect DLC. Black and green dial, Eco-Drive H800 radio-controlled movement. Sapphire crystal, 100 m WR. Original box and papers.";
...
async function main() {
  ...
  const slug = slugify(`${brandSlug}-attesa-light-in-black-${REFERENCE}`);
  ...
  try {
    ...
    const data = {
      ...
      liquidColor: "Black and green dial · luminous hands and markers",
      longevity: "Eco-Drive H800 solar · radio controlled",
