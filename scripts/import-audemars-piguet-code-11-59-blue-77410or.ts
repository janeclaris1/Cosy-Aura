/**
 * Import Audemars Piguet Code 11.59 Blue 77410OR.OO.A344CR.01
 *
 * Source: https://genevawatchco.com/products/audemars-piguet-code-11-59-blue-77410or-oo-a344cr-01
 *
 * Usage:
 *   npx tsx scripts/import-audemars-piguet-code-11-59-blue-77410or.ts
 *   npx tsx scripts/import-audemars-piguet-code-11-59-blue-77410or.ts --apply
 */
...
const PRODUCT_URL =
  "https://genevawatchco.com/products/audemars-piguet-code-11-59-blue-77410or-oo-a344cr-01";

const PRICE_GHS = 1450;
const STOCK_PER_BRANCH = 1;

const BRAND = "Audemars Piguet";
const MODEL = "Code 11.59 Blue";
const REFERENCE = "77410OR.OO.A344CR.01";
...
const LEAD = `The Code 11.59 in rose gold pairs AP's contemporary case architecture with a deep blue dial for a refined dress-watch statement. The 38 mm 18 ct rose gold case blends a round bezel with an octagonal mid-case and double-curved sapphire crystal — the signature Code 11.59 silhouette.

Automatic calibre 5900 drives hours, minutes, seconds, and date with a 60-hour power reserve. The blue dial contrasts warmly against rose gold, while a leather strap with folding clasp completes the look. Certified pre-owned in excellent condition with original box and papers.`;
...
function buildDescription(): string {
  return rebuildWatchDescriptionFromRecord({
    ...
    bottleDetail:
      "38 mm 18 ct rose gold · sapphire crystal · blue dial · 30 m WR",
    liquidColor: "Blue dial · applied markers",
...
- Case: 38 mm, 18 ct rose gold
- Dial: Blue
- Crystal: Sapphire
- Strap: Leather with fold clasp
- Movement: Automatic calibre 5900
- Power reserve: 60 hours
- Water resistance: 30 metres
- Condition: Excellent certified pre-owned
- Included: Original box and papers`,
  });
}

const CONDITION_REPORT =
  "Excellent certified pre-owned. 38 mm rose gold Code 11.59 case, blue dial, leather strap. Calibre 5900 automatic with 60-hour reserve. Sapphire crystal, 30 m WR. Original box and papers included.";
...
async function main() {
  ...
  const slug = slugify(`${brandSlug}-code-11-59-blue-${REFERENCE}`);
  ...
  console.log(`Source CHF: ${product.variants[0]?.price ?? "—"}`);
